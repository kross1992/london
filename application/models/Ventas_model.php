...
class Ventas_model extends CI_Model {
    ...
    public function get_ventas($codigo_venta = FALSE)
    {
            ...
            {
                $this->db->select('v.*, c.nombres, c.apellidos, c.cedula, tv.descripcion as tipo, v.iva, v.cuota, tv.id  as id_tipo_venta, v.periodicidad');
...
//            echo "<pre>";var_dump($query);echo "</pre>";exit;
    ...
    public function get_detalles_abono($codigo_venta = FALSE)
    {
            if ($codigo_venta === FALSE)
            {
                $this->db->select('da.*, tp.nombre');
                $this->db->from('detalles_abonos da');
                $this->db->join('tipo_pago tp', 'da.tipo_pago = tp.id', 'left');
            ...
            $this->db->from('detalles_abonos da');
            $this->db->join('tipo_pago tp', 'da.tipo_pago = tp.id', 'left');
            $this->db->where('da.codigo_venta', $codigo_venta);
            $this->db->order_by('da.id', 'ASC');
...
//            echo "<pre>";var_dump($query);echo "</pre>";exit;
    ...
    public function get_detalles_pago($codigo_venta = FALSE)
    {
            if ($codigo_venta === FALSE)
            {
                $this->db->select('dp.*, tp.nombre');
                $this->db->from('detalles_pago dp');
                $this->db->join('tipo_pago tp', 'dp.tipo_pago = tp.id', 'left');
            ...
            $this->db->from('detalles_pago dp');
            $this->db->join('tipo_pago tp', 'dp.tipo_pago = tp.id', 'left');
            $this->db->where('dp.codigo_venta', $codigo_venta);
            $this->db->order_by('dp.id', 'ASC');
...
//            echo "<pre>";var_dump($query);echo "</pre>";exit;
    ...
    public function set_ventas()
    {
        ...
        $data = array(
            ...
            'num_sistecredito' => $this->input->post('numero_sistecredito')
    ...
    public function set_detalles_venta()
    {
        ...
        for ($i=1; $i <= $cantDetalles ; $i++) {
            ...
            $data = array(
               ...
               'descuento' => $this->input->post('desc'.$i),
